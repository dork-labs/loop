...
import { Radio, Lightbulb, Map, CheckSquare, Activity } from 'lucide-react';
...
const TYPE_STYLES: Record<
  IssueType,
  { className: string; Icon: React.FC<{ className?: string }> }
> = {
...
export const issueColumns: ColumnDef<Issue>[] = [
  {
    ...
    size: 80,
    cell: ({ row }) => (
      <span className="text-muted-foreground font-mono text-xs">#{row.original.number}</span>
    ),
  },
  {
    accessorKey: 'title',
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return <Badge className={cn('border', STATUS_STYLES[status])}>{STATUS_LABELS[status]}</Badge>;
    },
  },
  {
    ...
    size: 80,
    cell: ({ row }) => (
      <span className="text-muted-foreground font-mono text-xs">P{row.original.priority}</span>
    ),
  },
  {
    id: 'project',
