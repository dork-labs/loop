...
function TimelineNode({ issue, isRoot = false }: TimelineNodeProps) {
  ...
  return (
    <div className={cn('relative', isRoot ? 'mb-3' : 'mb-4 last:mb-0')}>
      ...
      <div
        className={cn(
          'absolute -left-[1.625rem] top-[0.6rem] size-3 rounded-full ring-2 ring-neutral-900',
          dotClass,
        )}
      />
      <Link
      ...
      >
        ...
        <div className="mb-1.5 flex flex-wrap items-center gap-1.5">
          <Badge
            variant="outline"
            className={cn('text-xs', TYPE_BADGE_CLASS[issue.type])}
          >
            {issue.type}
          </Badge>
          <Badge
            variant="outline"
            className={cn('text-xs', STATUS_BADGE_CLASS[issue.status])}
          >
  ...
  return (
    <div className="relative border-l border-neutral-700 pl-6">
      ...
      <div className="mb-3 flex items-center justify-between">
        ...
        <Badge
          variant="outline"
          className={cn('text-xs', CHAIN_STATUS_CLASS[chainStatus])}
        >
          {chainStatus}
        </Badge>
      </div>
