...
describe('GET /api/dispatch/next', () => {
  ...
  it('filters by projectId when provided', async () => {
    const db = getTestDb();
    const [projA] = await db.insert(projects).values({ name: 'Project A' }).returning();
    const [projB] = await db.insert(projects).values({ name: 'Project B' }).returning();
    ...
    const app = buildApp();
    const res = await app.request(`/api/dispatch/next?projectId=${projB.id}`, {
      headers: AUTH_HEADER,
    });
  ...
  it('returns prompt=null when no matching templates exist', async () => {
    ...
    const db = getTestDb();
    await db.update(promptTemplates).set({ deletedAt: new Date() });
