...
describe('projects CRUD', () => {
  ...
  it('lists projects excluding soft-deleted', async () => {
    ...
    expect(data.map((p: { name: string }) => p.name).sort()).toEqual(['Project A', 'Project B']);
  });
  ...
  it('paginates results correctly', async () => {
    ...
    await db
      .insert(projects)
      .values([{ name: 'P1' }, { name: 'P2' }, { name: 'P3' }, { name: 'P4' }]);
  ...
  it('returns a project by ID with goal and issue counts', async () => {
    const db = getTestDb();
    const [goal] = await db.insert(goals).values({ title: 'Goal 1', status: 'active' }).returning();
  ...
  it('updates a project', async () => {
    const db = getTestDb();
    const [project] = await db.insert(projects).values({ name: 'Original' }).returning();
