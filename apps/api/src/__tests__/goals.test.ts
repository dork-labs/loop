...
describe('Goals CRUD', () => {
  ...
  it('lists goals excluding soft-deleted', async () => {
    ...
    expect(data.map((g: { title: string }) => g.title).sort()).toEqual(['Goal A', 'Goal B']);
  });

  it('paginates goals with limit and offset', async () => {
    const db = getTestDb();
    await db.insert(goals).values([{ title: 'Goal 1' }, { title: 'Goal 2' }, { title: 'Goal 3' }]);
  ...
  it('gets a goal by ID', async () => {
    const db = getTestDb();
    const [created] = await db.insert(goals).values({ title: 'Find me' }).returning();
  ...
  it('updates goal status to achieved', async () => {
    const db = getTestDb();
    const [created] = await db.insert(goals).values({ title: 'Almost done' }).returning();
  ...
  it('soft-deletes a goal and excludes it from subsequent GET', async () => {
    const db = getTestDb();
    const [created] = await db.insert(goals).values({ title: 'To be deleted' }).returning();
    ...
    const [row] = await db.select().from(goals).where(eq(goals.id, created.id));
