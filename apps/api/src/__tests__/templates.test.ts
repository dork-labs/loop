...
describe('template versions', () => {
  ...
  it('promotes a draft version to active and retires the previous', async () => {
    ...
    const promoteRes = await app.request(
      `/templates/${template.id}/versions/${v2.id}/promote`,
    ...
    const [retired] = await db
      .select()
      .from(promptVersions)
      .where(eq(promptVersions.id, v1.id))
  ...
  it('returns 422 when promoting an already active version', async () => {
    ...
    const promoteRes = await app.request(
      `/templates/${template.id}/versions/${v1.id}/promote`,
      { method: 'POST', headers: AUTH_HEADER },
    )
  ...
  it('returns 404 when promoting a non-existent version', async () => {
    ...
    const res = await app.request(
      `/templates/${template.id}/versions/nonexistent/promote`,
      { method: 'POST', headers: AUTH_HEADER },
    )
