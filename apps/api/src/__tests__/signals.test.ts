...
async function postSignal(app: Hono<TestAppEnv>, data: Record<string, unknown> = {}) {
  const body = {
    source: 'test-source',
    type: 'error',
    severity: 'medium',
...
describe('Signal Ingestion', () => {
  ...
  it('derives issue title from payload message', async () => {
    ...
    expect(body.data.issue.title).toBe('[sentry] exception: NullPointerException in UserService');
  });
  ...
  it('persists both signal and issue to the database', async () => {
    ...
    const dbSignals = await db.select().from(signals).where(eq(signals.id, body.data.signal.id));
    expect(dbSignals).toHaveLength(1);

    const dbIssues = await db.select().from(issues).where(eq(issues.id, body.data.issue.id));
