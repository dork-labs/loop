...
goalRoutes.get('/', zValidator('query', paginationSchema), async (c) => {
  ...
  const [data, totalResult] = await Promise.all([
    db.select().from(goals).where(isNull(goals.deletedAt)).limit(limit).offset(offset),
    db.select({ count: count() }).from(goals).where(isNull(goals.deletedAt)),
  ]);
...
goalRoutes.patch('/:id', zValidator('json', updateGoalSchema), async (c) => {
  ...
  const [updated] = await db.update(goals).set(body).where(eq(goals.id, id)).returning();
