...
<Callout type="info">
  This guide assumes you have Node.js 20+, npm, and Docker installed. No external database needed —
  Docker provides a local PostgreSQL instance.
</Callout>
...
The setup script installs dependencies, copies environment files with working defaults, starts a local PostgreSQL database via Docker, and runs migrations.
...
<Callout type="info">
  The `.env.example` files contain pre-filled defaults that work immediately for local development.
...
<Tabs items={['curl', 'JavaScript']}>
  <Tab value="curl">```bash curl http://localhost:4242/health ```</Tab>
  <Tab value="JavaScript">
    ```js const res = await fetch("http://localhost:4242/health"); const data = await res.json();
    console.log(data); ```
  </Tab>
</Tabs>
...
<Callout type="info">
  Dispatch uses `FOR UPDATE SKIP LOCKED` to prevent multiple agents from claiming the same issue. If
  no issues are available, you receive a `204 No Content` response.
</Callout>
