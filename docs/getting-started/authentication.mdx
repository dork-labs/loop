...
The easiest way to generate a key is with the setup script (`npm run setup`), which writes the key to both `.env` files automatically. You can also generate one manually:
