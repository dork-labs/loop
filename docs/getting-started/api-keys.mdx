...
For local development, the `npm run setup` command generates a real `loop_`-prefixed key. If you want to use a simpler key during development, any non-empty string works -- format validation is not enforced. However, using the standard format is recommended to match production behavior.
