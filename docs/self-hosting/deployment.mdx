...
<Callout type="warn">
  The dashboard embeds the API key into the client-side bundle at build time. Only deploy the
  dashboard behind authentication or restrict access to trusted networks.
</Callout>
...
<Callout type="info">
  In production, use a process manager like [PM2](https://pm2.keymetrics.io/) or systemd to keep the
  API running and restart it on failure.
</Callout>
