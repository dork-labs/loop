...
Loop uses environment variables for all configuration. Each app validates its environment variables at startup using Zod — missing or invalid variables produce clear, formatted error messages instead of cryptic runtime errors.
...
The local default connects to the PostgreSQL container started by `npm run dx`.
