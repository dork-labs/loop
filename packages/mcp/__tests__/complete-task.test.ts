...
function createMockClient(overrides?: {
  ...
  return {
    patch: vi.fn().mockReturnValue({
      json: overrides?.patchJson ?? vi.fn().mockResolvedValue({ data: { id: 'issue-1', status: 'done' } }),
    }),
    post: vi.fn().mockReturnValue({
      json: overrides?.postJson ?? vi.fn().mockResolvedValue({ data: { id: 'comment-1', body: 'Done' } }),
    }),
    get: vi.fn().mockReturnValue({
      json: overrides?.getJson ?? vi.fn().mockResolvedValue({
        data: [
          {
            id: 'issue-2',
            status: 'todo',
...
async function callWithClient(
  client: ApiClient,
  params: { issueId: string; outcome: string },
) {
...
describe('registerCompleteTask', () => {
  ...
  it('returns not-found error when PATCH returns 404', async () => {
    const client = createMockClient({
      patchJson: vi.fn().mockRejectedValue(
